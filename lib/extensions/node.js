...
exports.node_createContainer = function (results) {
	...
	console.log('Setting', self._id, 'to', backend.server.app_memory * 1024 * 1024, 'bytes of ram (', backend.server.app_memory, 'MB )');
...
exports.node_startContainer = function (results) {
	...
	var binds = [];
	binds.push('/home/ng_users/'+results.app.user+'/'+self._id+'/'+":/srv/app");
	binds.push('/home/ng_users/'+results.app.user+'/.ssh:/root/.ssh');
...
exports.getSystemUser = function(cb) {
	...
	models.App.findOne({
		...
		console.log('[INFO] '+__dirname+'/../scripts/getUser.sh '+user);
		var update = exec(__dirname+"/../scripts/getUser.sh "+user);
		update.on('close', function(code) {
			if (code !== 0) {
				console.log("User Could not be Created!", code);
			}
