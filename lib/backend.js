	...
	models.Server.findOne({
		...
		if (!server) {
			...
			server = new models.Server({
				name: config.credentials.server.name,
				location: config.credentials.server.address,
				identifier: config.credentials.server.id,
				price_per_hour: config.credentials.server.price,
				app_memory: config.credentials.server.app_memory
			});
			server.save();
		}
