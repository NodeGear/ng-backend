		...
		socket.on('auth', function(data) {
			...
			User.authenticate(data.token, function(user) {
				if (user != null) {
					...
					// Find drones and attach this io to them.
					for (var i = 0; i < sockets.length; i++) {
						if (sockets[i].user && sockets[i].user._id.equals(user._id)) {
							// yay
							for (var ii = 0; ii < sockets[i].drones.length; ii++) {
								sockets[i].drones[ii].io = socket;
							}
						}
					}
			...
			User.authenticate(data.token, function(user) {
				if (user != null) {
					droneAPI.findDrone(data, function(drone) {
						if (drone != null) {
							if (drone.model.isRunning == true) {
								// Already running!
								socket.emit('dronedata', "Drone already running. Stop/restart if you want")
								return;
							}
