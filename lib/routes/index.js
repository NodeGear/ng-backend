...
function createDrone(data) {
	getDrone(this, data, function(process, drone) {
		...
		drone.populate('user', function() {
			...
			var template = config.templateLocation + data.template.toLowerCase() + "/";
			
			var args = [
				drone.location,
				drone.user._id,
				config.droneLocation+drone.user._id+"/",
				template
			];
			var script = config.path+"/scripts/create_template_drone.sh "+args.join(" ");
...
function endHook (channel, drone) {
	...
	server.redis_client.publish(channel, JSON.stringify({
		message: msg.join("\n"),
		exit: true
	}));
}