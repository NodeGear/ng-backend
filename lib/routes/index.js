...
function gitHook (data) {
	...
	models.User.findById(uid, function(err, user) {
		...
		models.Drone.findOne({
			...
			if (!drone) {
				socket.emit('git hook response', "Sorry, we cannot really create new drones via git yet.. Apologies.");
				socket.emit('git hook end');
				return;
			}
			...
			getDrone(null, {
				...
				run.stdout.on('data', function(data) {
					socket.emit('git hook respone', data)
				})
				...
				run.on('close', function(code) {
					...
					socket.emit('git hook response', "Application running on subdomain "+drone.pkg.subdomain+".nodecloud.net");
