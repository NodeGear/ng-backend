...
var is_mac = require('os').platform() == 'darwin';
...
Process.prototype.unamanageProcess = function() {
	var self = this;
		...
		models.App.findOne({
			...
			client.lrange('pm:app_process_log_'+self.current_log, 0, 19, function(err, lines) {
				...
				app.user.sendEmailText(
					"Nodegear PM Daemon <notifications@ndoegear.com>",
...
Process.prototype.processOutput = function(chunk, prefix) {
	...
	client.lindex('pm:app_process_logs_'+self._id, 0, function(err, log_file) {
		if (err) throw err;

		if (log_file != self.current_log) {
			// Does not Exists
...
Process.prototype.launchProcess = function (cb) {
	...
	async.parallel({
		...
		app: function(done) {
			models.App.findOne({
				_id: self.app_id
			}).select('script user location').exec(done);
		},
	...
	models.AppDomain.find({
		...
		for (var i = 0; i < domains.length; i++) {
			...
			if (domain.is_subdomain) {
				doms[domain.domain+'.ngapp.io'] = self.app_id;
			} else {
...
Process.prototype.removeFromProxy = function() {
	...
	// TODO. CHeck if this process is last in the list. If it is, delete the proxy:app_ list. If it is, also remove all domains from proxy:domains.
	// This is when the scenario above isn't true
...
Process.prototype.installProcess = function(app, cb) {
	...
	var install = exec(__dirname+"/scripts/installProcess.sh "+user+" "+self._id+" \""+app.location+"\" \""+branch+"\"");
	install.on('close', function(code) {
		...
		if (process.env.NG_TEST || is_mac) {
			self.app_location = '/Users/matejkramny/'+self._id+'/';
		}
...
Process.prototype.cleanProcess = function() {
	...
	models.App.findOne({
		...
		var update = exec(__dirname+"/scripts/cleanProcess.sh "+app.user+" \""+self.app_location+"\"");
		update.on('close', function(code) {
			console.log(code);
			if (code === 0) {
