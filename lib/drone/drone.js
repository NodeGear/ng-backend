...
Drone.prototype.validatePackage = function () {
	...
	if (this.pkg.start == null || this.pkg.start.length == 0) {
		// Raise error
		
		return "No specified start in package.json";
	}
...
Drone.prototype.start = function (cb) {
	...
	// Start this drone.
	if (this.model.isInstalled == false) {
		// Cannot start because not installed.
		console.log("Drone not installed")
		cloud.emit("drone:data", this, "Drone not installed yet.");
	...
	async.series([
		...
		function(cb) {
			self.installDependencies(function(err) {
				...
				cloud.emit("drone:data", self, "Dependencies installed");
				
				self.model.isInstalled = true;
				self.model.save(function(err) {
					if (err) throw err;
				});
...
Drone.prototype.log = function (data) {
	console.log(data)
	if (typeof this.io !== "undefined" && typeof this.io.emit !== 'undefined') {
