...
var Proxy = function() {
	...
	this.wsRequest = function (req, socket, head) {
		...
		console.log("Proxying WS to ", nextProc)
	...
	this.httpRequest = function(req, res) {
		...
		if (drone.processes.length == 0) {
			// Display 404 - no ACTIVE drones found
	...
	for (var i = 0; i < self.list.length; i++) {
		if (self.list[i]._id.equals(drone._id)) {
			...
			for (var i = 0; i < item.processes.length; i++) {
				if (item.processes[i].pid == process.pid) {
					// Remove this process;
...
Proxy.prototype.removeDrone = function (process, drone) {
	var self = this;
