...
var Proxy = function() {
	...
	this.httpRequest = function(req, res) {
		...
		var lastAccess = drone.lastAccess;
		if (lastAccess == -1 || lastAccess > drone.processes.length) {
			lastAccess = 0;
		}
...
Proxy.prototype.proxyDrone = function(drone, processes) {
	var self = this;
	...
	for (var i = 0; i < processes.length; i++) {
		// increment the port.. should be checked that its in use
...
Proxy.prototype.removeProcess = function (process, drone) {
	var self = this;
...
Proxy.prototype.updatePort = function (drone) {
	drone.port = this.lastUsedPort++;
	for (var i = 0; i < this.list.length; i++) {
		if (this.list[i]._id.equals(drone._id)) {
			this.list[i].port = drone.port;
		}
	}
}
